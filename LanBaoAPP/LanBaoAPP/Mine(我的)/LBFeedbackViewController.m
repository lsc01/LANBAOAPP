...
@interface LBFeedbackViewController ()<UITableViewDelegate,UITableViewDataSource,UIActionSheetDelegate,UINavigationControllerDelegate, UIImagePickerControllerDelegate,TZImagePickerControllerDelegate>
...
-(UITableView *)tableView{
    if (_tableView == nil) {
        ...
        [_tableView registerClass:[LBTableViewCell class] forCellReuseIdentifier:@"LBTableViewCell"];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    switch (indexPath.section) {
        case 0:
        {
            LBTableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:@"LBTableViewCell" forIndexPath:indexPath];
            //点击添加图片
            WeakSelf();
            [cell setPhotoSelectBlock:^(NSInteger count , LBTableViewCell * cellT) {
                StrongSelf();
                //选择图片
                __weak typeof(cellT) weakCell = cellT;
                [strongself openPhotoMenuWithLimit:4-count Photos:^(NSArray *photoArray) {
...
-(void)getDataWithAllCell{
    LBTableViewCell * cell3 = [self.tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0]];
