...
+ (NSString *)getCacheSize {
    ...
    NSString *totleSize = [WLClearCacheTool getCacheSizeWithFilePath:cachesPath];
    return totleSize;
}
...
+ (BOOL)clearCache {
    NSString *cachesPath = [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) firstObject];
    BOOL isAlreadyClearCache = [WLClearCacheTool clearCacheWithFilePath:cachesPath];
    return isAlreadyClearCache;
}
+ (void)readyClearCache {
    BOOL isClearCache = [WLClearCacheTool clearCache];
    if (isClearCache) {
        NSLog(@"清理完毕");
    } else {
        NSLog(@"清理失败");
...
+ (NSString *)getCacheSizeWithFilePath:(NSString *)path {
    ...
    for (NSString *subPath in subPathArr){
        ...
        BOOL isExist = [[NSFileManager defaultManager] fileExistsAtPath:filePath isDirectory:&isDirectory];
        // 4. 以上判断目的是忽略不需要计算的文件
        if (!isExist || isDirectory || [filePath containsString:@".DS"] || [subPath containsString:@"Snapshots"]){
            // 过滤: 1. 文件夹不存在  2. 过滤文件夹  3. 隐藏文件 4.把Snapshots文件夹过滤掉,没有访问权限
            continue;
        }
        // 5. 指定路径，获取这个路径的属性
        NSDictionary *dict = [[NSFileManager defaultManager] attributesOfItemAtPath:filePath error:nil];
